  ...
  return (
    <div className="min-h-screen bg-[#013220] flex flex-col md:flex-row">
      ...
      <div className="relative">
        ...
        <header className="fixed top-0 left-0 w-full z-50 bg-[#012615]/90 backdrop-blur-xl border-b border-green-800/40 shadow-lg flex items-center justify-between px-4 py-3 lg:hidden">
          {/*  Animated Hamburger */}
          <motion.button
            whileHover={{ scale: 1.1 }}
            whileTap={{ scale: 0.9 }}
            onClick={() => setIsSidebarOpen(!isSidebarOpen)}
            className="text-white bg-gradient-to-r from-green-600 to-emerald-600 p-3 rounded-xl shadow-xl border border-green-500/30"
          ...
          <Link href="/" className="flex items-center gap-2">
            <motion.img
              ...
              className="w-10 h-10"
            />
            <span className="text-[#00ff88] font-bold text-lg tracking-wide">
              PixelMint
            </span>
          </Link>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={handleLogout}
            className="text-red-400 bg-red-900/40 px-3 py-2 rounded-lg border border-red-600/40 hover:bg-red-900/60 transition-all duration-300"
          >
            Logout
          </motion.button>
        </header>
      ...
      {/* Main Content */}
      <main className="flex-1 p-4 md:p-8 text-white mt-16 lg:mt-0">
        {activePage === "profile" && (
